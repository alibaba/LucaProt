...
with open(os.path.join(dir_path, "ecology/all_lib_rpm_cov.txt.cov20.res_v4.csv"), "w") as wfp:
        ...
        for row in reader:
            ...
            if lib in sra_lat_lon_info:
                ...
                if lat_lon not in lat_lon_species:
                    lat_lon_species[lat_lon] = {"unknow": set(), "know": set()}
...
with open("map/data_v4.csv", "w") as wfp:
    writer = csv.writer(wfp)
    writer.writerow(["lat", "lon", "know_species_num", "unknow_species_num", "species_num"])
    cnt = 1
    for item in data.items():
        ...
        unknow_species_num = item[1]["unknow"]
        assert len(lat_lon_species_no_label[lat_lon]) == int(know_species_num + unknow_species_num)
        writer.writerow([lat, lon, know_species_num, unknow_species_num, know_species_num + unknow_species_num])
        cnt += 1
...
# load data
data = []
with open("map/data_v4.csv", "r") as rfp:
    reader = csv.reader(rfp)
    cnt = 0
    for row in reader:
        cnt += 1
        if cnt == 1:
            continue
        # print(row)
        lat, lon, know_species_num, unknow_species_num, species_num = row[0], row[1], row[2], row[3], row[4]
        data.append([lat, lon, know_species_num, unknow_species_num, species_num])

print("data size: %d" %len(data))
data = np.array(data)
df = pd.DataFrame(data, columns=["lat", "lon", "know_species_num", "unknow_species_num", "species_num"], dtype=float)
...
df.to_csv("map/data_dbscan_v4.csv", encoding='utf-8')

df = pd.read_csv("map/data_dbscan_v4.csv", delimiter=',')
...
for index, row in df.iterrows():
    ...
    unknow_species_num = row["unknow_species_num"]
    if label not in cluster_data:
        cluster_data[label] = []
    cluster_data[label].append([lat, lon, know_species_num, unknow_species_num, know_species_num + unknow_species_num])


new_data = []
species_num_list = []
with open("map/cluster_data_v4.csv", "w") as wfp:
    writer = csv.writer(wfp)
    writer.writerow(["label", "lat", "lon", "know_species_num", "unknow_species_num", "species_num"])
    for item1 in cluster_data.items():
        ...
        species_num = 0
        know_species_num = 0
        unknow_species_num = 0
        for item2 in item1[1]:
            species_num += item2[2]
            species_num += item2[3]
            know_species_num += item2[2]
            unknow_species_num += item2[3]
            avg_lat += item2[0]
            avg_lon += item2[1]
        avg_lat /= len(item1[1])
        avg_lon /= len(item1[1])
        assert int(species_num) == int(know_species_num + unknow_species_num)
        new_data.append([label, avg_lat, avg_lon, know_species_num, unknow_species_num, species_num])
        writer.writerow([label, avg_lat, avg_lon, know_species_num, unknow_species_num, species_num])
...
for item in new_data:
    ...
    unknow_species_num = int(item[4])
    species_num = int(item[5])
    assert species_num == know_species_num + unknow_species_num
    ...
    if species_num < 10:
        ...
        radius = 5
    radius /= 2
    if know_species_num <= 0:
        cur_nums = [unknow_species_num]
        cur_colors = ["#EF6A90"]
    elif unknow_species_num <= 0:
        cur_nums = [know_species_num]
        cur_colors = ["#D1D1D1"]
    else:
        cur_nums = [know_species_num, unknow_species_num]
        cur_colors =["#D1D1D1", "#EF6A90"]
    ax_sub.pie(cur_nums,
               wedgeprops={'edgecolor': '#333333', 'linewidth': 0.25, 'antialiased': True},
               radius=radius,
               colors=cur_colors)
plt.savefig('map_pie_rdrp_species_2groups_v4_no_trans_level_5.pdf', dpi=600, format="pdf")
plt.savefig('map_pie_rdrp_species_2groups_v4_no_trans_level_5.png', dpi=600, format="png")
plt.show()