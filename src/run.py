...
try:
    ...
    from utils import set_seed, save_labels, get_parameter_number
    ...
    from src.utils import set_seed, save_labels, get_parameter_number
...
def main():
    ...
    parser.add_argument("--tfrecords", action="store_true", help="whether the dataset is in the tfrecords. When true, only the specified number of samples(${shuffle_queue_size}) are loaded into memory at once, the tfrecords must  be in ${data_dir}/tfrecords/train/xxx.tfrecords, ${data_dir}/tfrecords/dev/xxx.tfrecords and ${data_dir}/tfrecords/test/xxx.tfrecords. xxx.tfrecords may be 01-of-01.tfrecords(only including sequence)、01-of-01_emb.records(including sequence and structural embedding)、01-of-01_pdb_emb.records(including sequence, 3d-structure contact map, and structural embedding)")
    parser.add_argument("--shuffle_queue_size", default=5000, type=int, help="how many samples are loaded into memory at once")
    parser.add_argument("--multi_tfrecords", action="store_true", help="whether exists multi-tfrecords")
    ...
    parser.add_argument("--model_type", default=None, type=str, required=True,  choices=["sequence", "structure", "embedding", "sefn", "ssfn"], help="model type selected in the list: ['sequence-based', 'structure-based', 'structural embedding based', 'sequence and structural embedding based', 'sequence and structure based']")
    parser.add_argument("--subword",  action="store_true", help="whether use subword-level for sequence")
    ...
    # for structure
    parser.add_argument("--cmap_type", default=None, type=str, choices=["C_alpha", "C_bert"], help="the calculation type of 3d-structure contact map")
    ...
    parser.add_argument("--seq_pooling_type",  type=str, default=None, choices=["none", "sum", "max", "avg", "attention", "context_attention", "weighted_attention", "value_attention", "transformer"],
                        help="pooling type for sequence encoder")
    # structure pooling_type
    parser.add_argument("--struct_pooling_type",  type=str, default=None, choices=["sum", "max", "avg", "attention", "context_attention", "weighted_attention", "value_attention", "transformer"],
    ...
    parser.add_argument("--max_metric_type",  type=str, default="f1", required=True, choices=["acc", "jaccard", "prec", "recall", "f1", "fmax", "roc_auc", "pr_auc"], help="which metric for model selected")

    # for BCE Loss
    parser.add_argument("--pos_weight", type=float, default=40, help="positive weight for bce")

    # for CE Loss
    parser.add_argument("--weight", type=str, default=None, help="positive weight for multi-class")
    ...
    parser.add_argument("--struct_max_length", default=2048, type=int, help="the length of input contact map more than max length will be truncated, shorter will be padded.")
    ...
    # for embedding input
    parser.add_argument("--embedding_input_size", default=2560, type=int, help="the length of input embedding dim.")
    parser.add_argument("--embedding_type", type=str, default="matrix", choices=[None, "contacts", "bos", "matrix"], help="the type of the structural embedding info")
    parser.add_argument("--embedding_max_length", default=2048, type=int, help="the length of input embedding more than max length will be truncated, shorter will be padded.")
    ...
    if args.do_train:
        ...
        global_step, tr_loss, max_metric_model_info = train(args, model, processor, seq_tokenizer, subword, struct_tokenizer=struct_tokenizer, log_fp=log_fp)
