...
@file: data_preprocess_for_rdrp2
@desc: xxxx
'''
import os
import numpy as np
...
sys.path.append("../../src")
try:
    from utils import write_fasta, fasta_reader, clean_seq, load_labels
except ImportError:
    from src.utils import write_fasta, fasta_reader, clean_seq, load_labels
...
class GenerateTFRecord(object):
    def __init__(
        ...
        if protein_2_embedding_index_filepath:
    ...
    def load_protein_2_idx(self, header=True):
        '''
        the mapping betwwen the protein id and index
        :return:
        '''
        protein_2_idx = {}
        with open(self.protein_2_embedding_index_filepath, "r") as rfp:
            ...
            for row in reader:
                ...
                if cnt == 1 and header:
                    continue
                index, protein = row[0], row[1]
                protein_2_idx[protein] = index
        return protein_2_idx
    ...
    def load_dataset(self, header=True):
        ...
        with open(self.dataset_filename, "r") as rfp:
            ...
            for row in reader:
                ...
                if cnt == 1 and header:
                    continue
                protein_id, seq, label, source = row
                seq = clean_seq(protein_id, seq)
                dataset[protein_id] = [seq, ]
        return dataset
    ...
    def _serialize_example(self, obj_id, sequence, embedding_obj, pdb_obj, label):
        d_feature = {}
