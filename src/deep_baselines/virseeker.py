...
class VirSeeker(nn.Module):
    ...
    def forward(self, x, lengths, labels=None):
        if self.embedding:
            ...
            x = torch.unsqueeze(x, 2).to(torch.float32)
        packed_input = nn.utils.rnn.pack_padded_sequence(x, lengths.to('cpu'), enforce_sorted=False, batch_first=self.batch_first)
