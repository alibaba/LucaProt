...
def multi_label_acc(targets, probs, threshold=0.5):
    ...
    for idx in range(targets.shape[0]):
        ...
        if union_len == 0:
            ...
            acc_list.append(acc)
    return round(sum(acc_list)/len(acc_list), 4) if len(acc_list) > 0 else 0
...
def multi_label_precision(targets, probs, threshold=0.5):
    ...
    round(sum(prec_list)/len(prec_list), 4) if len(prec_list) > 0 else 0
...
def multi_label_recall(targets, probs, threshold=0.5):
    ...
    return round(sum(recall_list)/len(recall_list), 4) if len(recall_list) > 0 else 0
...
def multi_label_jaccard(targets, probs, threshold=0.5):
    ...
    return round(sum(jaccard_list)/len(jaccard_list), 4) if len(jaccard_list) > 0 else 0
...
def multi_label_f1(targets, probs, threshold=0.5):
    ...
    for idx in range(targets.shape[0]):
        ...
        if union_len == 0:
            ...
            f1_list.append(f1)
    return round(sum(f1_list)/len(f1_list), 4) if len(f1_list) > 0 else 0
...
def multi_label_roc_auc(targets, probs, threshold=0.5):
    ...
    return round(sum(roc_auc_list)/len(roc_auc_list), 4) if len(roc_auc_list) > 0 else 0
...
def multi_label_pr_auc(targets, probs, threshold=0.5):
    ...
    return round(sum(pr_auc_list)/len(pr_auc_list), 4) if len(pr_auc_list) > 0 else 0
...
def metrics_multi_label_for_pred(targets,  preds, savepath=None):
    ...
    return {
        "acc": round(sum(acc_list)/targets.shape[0], 4),
        "jaccard": round(sum(jaccard_list)/targets.shape[0], 4),
        "prec": round(sum(prec_list)/targets.shape[0], 4),
        "recall": round(sum(recall_list)/targets.shape[0], 4),
        "f1": round(sum(f1_list)/targets.shape[0], 4)
    }
